...
ap.add_argument("--versiones", nargs="+", default=["viejo", "nuevo"],
                choices=["viejo", "nuevo"])
ap.add_argument("--tree-random-init", action="store_true",
                help="usa el brazo arreglado del NODE; por omision se usa el "
                     "publicado, con los parametros de arbol en cero. No afecta "
                     "a los demas embedders")
ap.add_argument("--commit", default="336769f",
...
def engancha(mod, reloj, overrides):
    ...
    las dos. El mismo envoltorio inyecta los overrides del brazo del NODE, que la
    version vieja no sabe recibir por configuracion.
...
# --- la rejilla ---
YA = hechas()
brazo = "random" if a.tree_random_init else "zero"
...
print(f"[rejilla] embedders {' '.join(a.embedders)} (node en brazo {brazo}) | "
...
for ds in a.datasets:
    ...
    for emb in a.embedders:
        # El brazo solo existe para el NODE; pasarselo a los demas seria un
        # override que su constructor no conoce.
        overrides = ({"tree_random_init": True}
                     if (a.tree_random_init and emb == "node") else None)
        for ipc in a.ipcs:
            P = p_de(ipc)
            ...
            for s in range(a.seeds):
                ...
                for ver, mod in VERSIONES:
                    ...
                    suelta = engancha(mod, reloj, overrides)
                    set_seed(seed)
                    ...
                    fila = dict(version=ver, dataset=ds, tipo=tipo, C=C, d=d,
                                embedder=emb, brazo=brazo, ipc=ipc, p=P,
