...
Measured here per embedder family and size rung, repeated for an average:
  ...
  - that the NODE arms survive: with tree_random_init off the tree parameters go
    back to zero and the ensemble stays degenerate; with it on they are redrawn
...
# Los dos brazos del NODE tienen que sobrevivir al reciclado, o el experimento 3
# estaria comparando el mismo embedder consigo mismo.
print("\n=== brazos del NODE tras reiniciar ===")
for arm, rnd in [("zero", False), ("random", True)]:
    torch.manual_seed(0)
    net = sample_random_embedder("node", "base", 20, 256, 128, DEV,
                                 overrides={"tree_random_init": rnd})
    ens = [m for m in net.modules() if isinstance(m, ObliviousTreeEnsemble)][0]
    xx = torch.randn(512, 20, device=DEV)
    for k in range(3):
        reinit_embedder_(net)
        with torch.no_grad():
            h = net.ln(net.in_proj(xx))
            out = ens(h)
        sv = torch.linalg.svdvals(out - out.mean(0))
        rango = int((sv > 1e-3 * sv[0]).sum())
        filas_dist.append(dict(embedder=f"node_{arm}", ipc=-1, d=20, p=128,
                               modo="reiniciar", rep=k,
                               peso_std=float(ens.feature_logits.std()),
                               peso_media=float(ens.thresholds.std()),
                               salida_std=float(out.std()), salida_media=float(sv[0]),
                               salida_min=rango, salida_max=ens.tree_dim))
        print(f"  {arm:6s} reinit {k+1}: logits std {ens.feature_logits.std():.4f}  "
              f"thr std {ens.thresholds.std():.4f}  rango {rango}/{ens.tree_dim}")
